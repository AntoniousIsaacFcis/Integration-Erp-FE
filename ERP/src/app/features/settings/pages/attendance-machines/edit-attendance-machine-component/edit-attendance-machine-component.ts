...
import { IUpdateAttendanceMachine } from '@features/settings/models/iattendance-machine';
...
@Component({
  selector: 'app-edit-attendance-machine-component',
  standalone: true,
  imports: [
    TranslocoModule,
    ReactiveFormsModule,
  ...
  templateUrl: './edit-attendance-machine-component.html',
  styleUrl: './edit-attendance-machine-component.css',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class EditAttendanceMachineComponent implements OnInit {
  ...
  ngOnInit() {
    this.attendanceMachineId = this.route.snapshot.paramMap.get('id') ?? '';

    if (!this.attendanceMachineId) {
      this.router.navigate(['/settings/attendance-machines']);
      return;
    }

    this.service
      .getById(this.attendanceMachineId)
      ...
      .subscribe({
        next: (attendanceMachine) => {
          this.attendanceMachineForm.patchValue({
            ...
            isActive: attendanceMachine.isActive,
          });
          this.isLoading.set(false);
        },
        error: (error) => {
          console.error('Failed to load attendance machine:', error);
          this.isLoading.set(false);
          this.router.navigate(['/settings/attendance-machines']);
        },
      });
  ...
  onSubmit() {
    ...
    const payload: IUpdateAttendanceMachine = {
    ...
    this.service
      .update(this.attendanceMachineId, payload)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
