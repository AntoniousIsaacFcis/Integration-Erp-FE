...
export class ViewShiftComponent {
  ...
  canCreateShift = computed(() => canManageShift(this.authService, SHIFT_PERMISSIONS.create));
  canEditShift = computed(() => canManageShift(this.authService, SHIFT_PERMISSIONS.update));
  canDeleteShift = computed(() => canManageShift(this.authService, SHIFT_PERMISSIONS.delete));
  ...
  shiftsList = computed(() => {
    ...
    return response.map((shift: IShiftListItem, index) => ({
      ...shift,
      displayName: `${shift.name || shift.nameAr || shift.nameEn || '-'} #${baseIndex + index + 1}`,
